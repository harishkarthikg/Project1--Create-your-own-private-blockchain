        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (curTime < (messageTime + (5 * 60 * 1000))) {
                ...
                if (isValid) {
                    ...
                    reject('ERR: Not a valid signature!')
                }
            } else {
                reject('ERR: A new star can be added only every five minutes or less')
            }
        });
    }
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (block !== undefined) {
                ...
                reject('ERR: Could not find a block with that particular hash!')
            }
        });
    }
        ...
        return new Promise(async(resolve,reject) => {
            if (self.height > 0) {
                for (var i=0; i<=self.height; i++) {
                    ...
                    if (!validation){
                        console.log(" ERR IN VALIDATING DATA");
                    } else if (block.previousBlockHash != self.chain[i-1].hash) {
                        console.log("ERR WITH THE PREVIOUS BLOCK HASH")
                    }
                }
                if (errorLog) {
                    resolve(errorLog);
